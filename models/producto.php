...
class producto extends Database{
    ...
    public function find($id){
        try{
            $result = parent::connect()->prepare("SELECT * FROM producto WHERE id_producto = ?");
    ...
    public function update_register($data){
        try{
            $result = parent::connect()->prepare("UPDATE producto SET nombre = ?, descripcion = ?, cantidad = ?, valor= ?, url_image = ? WHERE id_producto = ?");
            $result->bindParam(1, $data['nombre'], PDO::PARAM_STR);
            $result->bindParam(2, $data['descripcion'], PDO::PARAM_STR);
            $result->bindParam(3, $data['cantidad'], PDO::PARAM_INT);
            $result->bindParam(4, $data['valor'], PDO::PARAM_INT);
            $result->bindParam(5, $data['url_image'], PDO::PARAM_STR);
            $result->bindParam(6, $data['id_producto'], PDO::PARAM_INT);
            return $result->execute();
        }catch (Exception $e){
    ...
    public function delete_register($data){
        try{
            $result = parent::connect()->prepare("DELETE FROM producto WHERE id_producto = ?");
            $result->bindParam(1, $data['id_producto'], PDO::PARAM_INT);
            return $result->execute();
        }catch (Exception $e){
            die("Error producto->update_register() " . $e->getMessage());
        }
